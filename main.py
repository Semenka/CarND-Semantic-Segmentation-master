...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # Upsample layer 1
    layer_7_1x1_upsample=tf.layers.conv2d_transpose(layer_7_1x1,num_classes,5,2,padding='same', kernel_initializer = init,
                                      kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    layer1 = tf.layers.batch_normalization(layer_7_1x1_upsample)

    #Layer4 building
    layer_4_1x1=tf.layers.conv2d(vgg_layer4_out,num_classes,1,padding='same',  kernel_initializer = init,
                               kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    #Add skip connection from 1st layer to layer4
    layer1 = tf.add(layer1, layer_4_1x1)

    #Upsample result
    Layer1_upsample=tf.layers.conv2d_transpose(layer1,num_classes,5,2,padding='same',  kernel_initializer = init,
                                      kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    Layer2=tf.layers.batch_normalization(Layer1_upsample)

    #Layer 3 building
    layer_3_1x1=tf.layers.conv2d(vgg_layer3_out,num_classes,1,padding='same',  kernel_initializer = init,
                               kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    #Add second skip connection from layer 3
    Layer2 = tf.add(Layer2, layer_3_1x1)
    Layer2_upsample=tf.layers.conv2d_transpose(Layer2,num_classes,14,8,padding='same',  kernel_initializer = init,
                                      kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    tf.Print(Layer2_upsample,[tf.shape(Layer2_upsample)])
    return Layer2_upsample
